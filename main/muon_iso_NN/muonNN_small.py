...
#muonNN_small.py - Prompt vs Non-Prompt Muon NN Classifier, Fixed 10x10 Network
#copy of muonNN.py with hardcoded hidden widths and no per-epoch auc tracking
...
#network architecture: n_in inputs (105 with the default feature settings) -> 10 -> 10 -> 1 output
class MLP(nn.Module):
    def __init__(self, n_in):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(n_in, 10), nn.ReLU(),
...
def main():
    ...
    settings_data = "\n".join([
        ...
        _row("Network", f"MLP {X_train.shape[1]}->10->10->1, ReLU, {n_params} parameters"),
